					...
					while ((sIzquierda = socketIzquierda.accept()) != null) {
						...
						try {
							...
							try {
								...
								BufferedReader readerMatriculas = new BufferedReader(
										new FileReader("./src/cameraRing/matriculas.txt"));
								...
								if (masterNode) {
									try {
										...
										ObjectOutputStream outputCentralServer = new ObjectOutputStream(
												socketCentralServer.getOutputStream());
										...
										time = ProxyClock.getError();
										Log.log(INFO, logPath, "Central Server Fallen, conecting to Server Backup",
												className, time);

										Socket socketCentralServerBackup = new Socket(ipCentralServerBackup,
												puertoCentralServerBackup);
										ObjectOutputStream outputCentralServerBackup = new ObjectOutputStream(
												socketCentralServerBackup.getOutputStream());
								...
								BufferedWriter bw = new BufferedWriter(
										new FileWriter("./src/cameraRing/matriculas.txt"));
								bw.write("");
								bw.close();
								...
								Socket socketDerecha = new Socket(ipDerecha, puertoDerecha);
								ObjectOutputStream outputDerecha = new ObjectOutputStream(
										socketDerecha.getOutputStream());
								outputDerecha.writeObject(message);
					...
					try {
						...
						while (!ringClosed) {
							...
							try {
								...
								ObjectOutputStream outputDerechaAux = new ObjectOutputStream(
										socketDerechaAux.getOutputStream());
					...
					try {
						...
						if (fallConfiguration.getCloseRing()) {
							...
							try (Socket socketIzquierda = new Socket(ipIzquierda, puertoIzquierda2);) {
								...
								ObjectOutputStream outputIzquierda = new ObjectOutputStream(
										socketIzquierda.getOutputStream());
								outputIzquierda.writeObject(fallConfiguration);
								...
								try {
									try {
										Process proc = Runtime.getRuntime()
												.exec("./src/cameraRing/corbaMatriculasDetector/clientCorba.bat "
														+ detectionCorbaPath + " " + matriculasCentralServerPath);
										...
										logLock.lock();
										try {
											Log.log(INFO, logPath, logInOut + ": " + matricula + " TIME: " + time,
													className, time);
										} finally {
											logLock.unlock();
										}
										...
										Thread.sleep(2000);
										System.out.println(
												"RECEIVED LICENSE PLATE: " + matricula + " FROM CORBA SERVER\n");
										...
										try {
											...
											if (!matriculaExist) {
												BufferedWriter bw = new BufferedWriter(
														new FileWriter("./src/cameraRing/matriculas.txt", true));
												bw.write(matricula + "\n");
												bw.close();
											}
